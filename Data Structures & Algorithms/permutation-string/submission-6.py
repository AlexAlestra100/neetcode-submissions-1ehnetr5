class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for r in range(len(s1), len(s2)):
            ...
            char_r = s2[r]
            s2_map[char_r] = s2_map.get(char_r, 0) + 1

            if s1_map.get(char_r, 0) == s2_map[char_r]:
                matches += 1
            elif s1_map.get(char_r, 0) + 1 == s2_map[char_r]:
                matches -= 1
            ...
            if s1_map.get(char_l, 0) == s2_map[char_l]:
                matches += 1
            elif s1_map.get(char_l, 0) - 1 == s2_map[char_l]:
                matches -= 1
